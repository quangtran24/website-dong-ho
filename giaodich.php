...
for($i = 0; $i < count($sl); $i++){
    ...
    $x = floatval($x);
    $money = floatval($money); // Convert $money to float if it's a string
    $money += $sl[$i] * $x;
...
if($_SESSION['rights'] == "user"){
	$user_id = $_SESSION['user']['MẵNgườiDùng'];
}

$sql = "INSERT INTO ĐơnHàng VALUES ('', '".$user_id."', '".$now."', '".$money."', 'Đã đặt')";
if(!mysqli_query($conn, $sql)){
	echo "Đã xảy ra một lỗi nhỏ trong quá trình đặt hàng, vui lòng đặt hàng lại!";
}

$sql = "SELECT MãĐơnHàng FROM ĐơnHàng WHERE MãĐơnHàng = LAST_INSERT_ID()";
...
if(isset($_SESSION['buynow'])){
	...
	$sql = "INSERT INTO ChiTiếtĐơnHàng VALUES ('".$last_maDH."', '".$buynow."', '".$sl[0]."')";
	require_once 'layout/second_footer.php';
	return 0;
}
...
for($i = 0; $i < count($new_masp); $i++){
	$sql_multi[] = "INSERT INTO ChiTiếtĐơnHàng VALUES ('".$last_maDH."', '".$new_masp[$i]."', '".$sl[$i]."')";
}
