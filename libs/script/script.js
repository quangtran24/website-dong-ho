...
function removeFromCart(masp) {
	// Xóa sản phẩm khỏi giỏ hàng trên máy chủ
	$.ajax({
	  url: 'order.php', // Đường dẫn tới file xử lý xóa sản phẩm khỏi giỏ hàng trên máy chủ
	  method: 'POST',
	  data: { masp: masp },
	  success: function(response) {
		// Xóa sản phẩm khỏi DOM
		var row = document.getElementById('row-' + masp);
		if (row) {
		  row.parentNode.removeChild(row);
		  tinh_tien(); // Cập nhật tổng tiền
		}
	  },
